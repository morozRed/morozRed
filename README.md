# 👋 Hey, I'm Grig

Software engineer. Building things, breaking things, shipping things.

I write about shipping fast, making tradeoffs, and the messy reality of building products at [morozred.com](https://morozred.com).

---

### What I'm building

**[JotChain](https://jotchain.app)** — Note-taking with scheduled summaries and workflow insights
`Rails` `AI` `Productivity`

**[Waytale](https://waytale.app)** — AI-powered audio guides that turn city walks into adventures
`Rails` `AI` `Travel`

---

### Recent writing

<!-- Update these manually or set up a GitHub Action -->
- [Shipping speed and LLM-driven development](https://morozred.com)
- [In-memory caching: when good enough is good enough](https://morozred.com)
- [Building under pressure](https://morozred.com)

---

### Say hi

[X](https://x.com/grigorymoroz) · [GitHub](https://github.com/morozred) · [grig@morozred.com](mailto:grig@morozred.com)
